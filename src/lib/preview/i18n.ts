...
const EN: LocaleStrings = {
  ...
  // See the note on the Slovenian aboutCopy: states only what Google verifies.
  aboutCopy: (area, label, reviewCount) =>
    `${area ? `You'll find our ${label} in ${area}.` : `A local ${label}.`}${
      reviewCount ? ` ${reviewCount} customers have reviewed us on Google.` : ""
    } We'd love to welcome you.`,
  // Neutral by design: "Trusted" / "Established" are self-awarded claims.
  eyebrows: ["Local", "Visit us", "Welcome", "Find us"],
  // "Quietly the best {label} in {area}" and "the {label} locals recommend" were
  // both unverified claims presented as fact — see copyPolicy.ts.
  taglines: [
    (label, area) => (area ? `Your local ${label} in ${area}.` : `Your local ${label}.`),
    (label, area) => (area ? `An everyday ${label} in ${area}.` : `An everyday ${label}.`),
    (label) => `Welcome to our ${label}.`,
  ],
  label: {
    salon: "salon",
    cafe: "café",
...
const SL: LocaleStrings = {
  ...
  // Only states what Google actually tells us: where the business is and how many
  // reviews it has. No "more than N" (it was printing that for exactly N), no
  // claims about returning customers or fair prices — nobody verified those.
  aboutCopy: (area, label, reviewCount) =>
    `${area ? `Naš ${label} najdete v kraju ${area}.` : `Lokalni ${label}.`}${
      reviewCount ? ` Na Googlu nas je ocenilo ${reviewCount} strank.` : ""
    } Veseli bomo vašega obiska.`,
  // Neutral by design — see the English note above.
  eyebrows: ["Lokalno", "Obiščite nas", "Dobrodošli", "Kje smo"],
  // Every form here is an appositive after a dash rather than a preposition, so
  // no Slovenian case declension is ever needed — "v Ljubljani" but "v Mariboru",
  // "v Celju", and the generator only has the nominative from Google. The old
  // second line read "salon po pravilih stroke – tukaj, Ljubljana", which is not
  // a sentence. The old third claimed the business was the best in town, which
  // nobody verified (see copyPolicy.ts).
  taglines: [
    (label, area) => (area ? `Vaš lokalni ${label} – ${area}.` : `Vaš lokalni ${label}.`),
    (label, area) => (area ? `${label} za vsakdan – ${area}.` : `${label} za vsakdan.`),
    (label) => `Dobrodošli v naš ${label}.`,
  ],
  label: {
    salon: "salon",
    cafe: "kavarna",
