...
//   { action: "send" } — deliver it now (real SMTP send or a Gmail compose link),
//                        mark it sent, and schedule the next step. Re-checks the
//                        signal gate server-side so a follow-up can never go to a
//                        prospect who has since replied or shown interest.
...
const BodySchema = z.object({ action: z.enum(["send", "skip"]) });
...
export async function POST(
  ...
  const result = await deliverOutreach(id);
  if (!result.ok) {
    return NextResponse.json(
